...
void FARMaster::Init() {
  ...
  /*init path generation thread callback*/
...
void FARMaster::PlanningCallBack(const ros::TimerEvent& event) {
  ...
  if (goal_ptr == nullptr) {
    ...
    // Update goal position with nearby terrain cloud
...
void FARMaster::OdomCallBack(const nav_msgs::OdometryConstPtr& msg) {
  ...
  //std::cout<<"\033[1;33m **************** is_odom_init_ and UpdateRobotPosition TRUE**************** \033[0m\n"<<std::endl;

  is_odom_init_ = true;
  //print  is_odom_init_
  if (is_odom_init_)
    std::cout<<"\033[1;33m **************** is_odom_init_ is TRUE **************** \033[0m\n"<<std::endl;
  else
    std::cout<<"\033[1;33m **************** is_odom_init_ is FALSE **************** \033[0m\n"<<std::endl;
...
void FARMaster::TerrainCallBack(const sensor_msgs::PointCloud2ConstPtr& pc) {
  ...
  if (!is_stop_update_) {
    ...
    // size temp_cloud_ptr_
    if (temp_cloud_ptr_->empty()) {
      std::cout<<"\033[1;33m **************** temp_cloud_ptr_ is EMPTY **************** \033[0m\n"<<std::endl;
    }
    ...
    // size temp_cloud_ptr_
    if (temp_cloud_ptr_->empty()) {
      std::cout<<"\033[1;33m ****************CropBoxCloud  temp_cloud_ptr_ is EMPTY **************** \033[0m\n"<<std::endl;
    }
    ...
    // size of temp_obs_ptr_ and temp_free_ptr_
    std::cout << "temp_obs_ptr_ size: " << temp_obs_ptr_->size() << std::endl;
    std::cout << "temp_free_ptr_ size: " << temp_free_ptr_->size() << std::endl;
    // extract new points
  ...
  std::cout<<"\033[1;33m **************** map_handler_  START **************** \033[0m\n"<<std::endl;
  std::cout << "START surround_obs_cloud_ size: " << FARUtil::surround_obs_cloud_->size() << std::endl;
  ...
  //check surround_free_cloud_, surround_obs_cloud_ and terrain_height_ptr_
  if (FARUtil::surround_free_cloud_->empty()) {
    std::cout<<"\033[1;33m **************** surround_free_cloud_ is EMPTY **************** \033[0m\n"<<std::endl;
  }
  //terrain_height_ptr_
  if (terrain_height_ptr_->empty()) {
    std::cout<<"\033[1;33m **************** terrain_height_ptr_ is EMPTY **************** \033[0m\n"<<std::endl;
  }

  std::cout << "GetSurroundObsCloud surround_obs_cloud_ size: " << FARUtil::surround_obs_cloud_->size() << std::endl;
  ...
  if (!master_params_.is_static_env && !is_stop_update_) {
    ...
    std::cout << "ExtractDynamicObsFromScan surround_obs_cloud_ size: " << FARUtil::surround_obs_cloud_->size() << std::endl;
    ...
    if (FARUtil::cur_dyobs_cloud_->size() > FARUtil::kDyObsThred) {
      ...
      std::cout << "RemoveOverlapCloud surround_obs_cloud_ size: " << FARUtil::surround_obs_cloud_->size() << std::endl;
  ...
  //size surround_obs_cloud_
  std::cout << "surround_obs_cloud_ size: " << FARUtil::surround_obs_cloud_->size() << std::endl;
  if (!FARUtil::surround_obs_cloud_->empty()) is_cloud_init_ = true;
  if (is_cloud_init_)
    std::cout<<"\033[1;33m **************** is_cloud_init_ is TRUE **************** \033[0m\n"<< std::endl;
  else
    std::cout<<"\033[1;33m **************** is_cloud_init_ is FALSE **************** \033[0m\n"<< std::endl;
