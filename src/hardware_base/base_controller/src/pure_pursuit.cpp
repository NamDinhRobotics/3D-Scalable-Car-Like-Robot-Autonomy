...
private:
    ...
    void computeControl() {
        ...
        const double transformed_x = dx * cos(-yaw) - dy * sin(-yaw);
        const double transformed_y = dx * sin(-yaw) + dy * cos(-yaw);
        ...
        const double curvature = 2.0 * transformed_y / (lookahead_distance_ * lookahead_distance_);
