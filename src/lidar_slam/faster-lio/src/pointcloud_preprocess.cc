...
void PointCloudPreprocess::Set(const LidarType lid_type, const double bld, const int pfilt_num) {
...
void PointCloudPreprocess::Process(const sensor_msgs::PointCloud2::ConstPtr &msg, const PointCloudType::Ptr &pcl_out) {
    switch (lidar_type_) {
...
void PointCloudPreprocess::AviaHandler(const livox_ros_driver::CustomMsg::ConstPtr &msg) {
        ...
        if ((msg->points[i].line < num_scans_) &&
            ...
            if (i % point_filter_num_ == 0) {
                ...
                cloud_full_[i].curvature =
                    ...
                    static_cast<float>(1000000);  // use curvature as time of each laser points, curvature unit: ms
    ...
    for (uint i = 1; i < plsize; i++) {
        // Calculate the squared distance to the sensor
        const float distance_squared = cloud_full_[i].x * cloud_full_[i].x +
                                 cloud_full_[i].y * cloud_full_[i].y +
                                 cloud_full_[i].z * cloud_full_[i].z;

        // Check if the point is valid and within a certain range
        if (is_valid_pt[i] && distance_squared > blind_ * blind_) {
...
void PointCloudPreprocess::Oust64Handler(const sensor_msgs::PointCloud2::ConstPtr &msg) {
    ...
    for (int i = 0; i < pl_orig.points.size(); i++) {
        ...
        const double range = pl_orig.points[i].x * pl_orig.points[i].x + pl_orig.points[i].y * pl_orig.points[i].y +
...
void PointCloudPreprocess::VelodyneHandler(const sensor_msgs::PointCloud2::ConstPtr &msg) {
    ...
    if (pl_orig.points[plsize - 1].time > 0) {
        ...
        const double yaw_first = atan2(pl_orig.points[0].y, pl_orig.points[0].x) * 57.29578;
        double yaw_end = yaw_first;
        const int layer_first = pl_orig.points[0].ring;
