...
bool LaserMapping::LoadParams(const ros::NodeHandle &nh) {
...
void LaserMapping::SubAndPubToROS(ros::NodeHandle &nh) {
    ...
    pub_odom_aft_mapped_ = nh.advertise<nav_msgs::Odometry>("/odometry_lio", 100000);
    pub_path_ = nh.advertise<nav_msgs::Path>("/path_lio", 100000);
}
...
void LaserMapping::Run() {
    ...
    const int cur_pts = scan_down_body_->size();
    if (cur_pts < 5) {
...
bool LaserMapping::SyncPackages() {
    ...
    if (!lidar_pushed_) {
        ...
        } else if (measures_.lidar_->points.back().curvature / static_cast<double>(1000) < 0.5 * lidar_mean_scantime_) {
            lidar_end_time_ = measures_.lidar_bag_time_ + lidar_mean_scantime_;
        } else {
            scan_num_++;
            lidar_end_time_ = measures_.lidar_bag_time_ + measures_.lidar_->points.back().curvature / static_cast<double>(1000);
            lidar_mean_scantime_ +=
                (measures_.lidar_->points.back().curvature / static_cast<double>(1000) - lidar_mean_scantime_) / scan_num_;
        }
...
void LaserMapping::MapIncremental() const {
 ...
 */
void LaserMapping::ObsModel(const state_ikfom &s, esekfom::dyn_share_datastruct<double> &ekfom_data) {
    const int cnt_pts = scan_down_body_->size();
    ...
    Timer::Evaluate(
        [&, this]() {
            ...
            std::for_each(std::execution::par_unseq, index.begin(), index.end(), [&](const size_t &i) {
                ...
                /* transform to world frame */
                const common::V3F p_body = point_body.getVector3fMap();
    ...
    Timer::Evaluate(
        [&, this]() {
            ...
            std::for_each(std::execution::par_unseq, index.begin(), index.end(), [&](const size_t &i) {
                const common::V3F point_this_be = corr_pts_[i].head<3>();
                const common::M3F point_be_crossmat = SKEW_SYM_MATRIX(point_this_be);
                const common::V3F point_this = off_R * point_this_be + off_t;
...
void LaserMapping::PublishFrameBody(const ros::Publisher &pub_laser_cloud_body) {
    const int size = scan_undistort_->points.size();
    const PointCloudType::Ptr laser_cloud_imu_body(new PointCloudType(size, 1));
...
void LaserMapping::PublishFrameEffectWorld(const ros::Publisher &pub_laser_cloud_effect_world) {
    const int size = corr_pts_.size();
    const PointCloudType::Ptr laser_cloud(new PointCloudType(size, 1));
...
void LaserMapping::PointBodyToWorld(const PointType *pi, PointType *const po) const {
...
void LaserMapping::PointBodyToWorld(const common::V3F &pi, PointType *const po) const {
    common::V3D p_body(pi.x(), pi.y(), pi.z());
    common::V3D p_global(state_point_.rot * (state_point_.offset_R_L_I * p_body + state_point_.offset_T_L_I) +
                         state_point_.pos);
...
void LaserMapping::PointBodyLidarToIMU(PointType const *const pi, PointType *const po) const {
...
void LaserMapping::Finish() const {
    /**************** save map ****************/
    /* 1. make sure you have enough memories
    /* 2. pcd save will largely influence the real-time performences **/
    if (pcl_wait_save_->size() > 0 && pcd_save_en_) {
        const auto file_name = std::string("scans.pcd");
        const std::string all_points_dir(std::string(std::string(ROOT_DIR) + "PCD/") + file_name);
        pcl::PCDWriter pcd_writer;
