...
double min_speed_{1.0}; // Minimum speed to prevent the vehicle from stopping
double max_steering_angle_{M_PI/6};
...
int main(int argc, char **argv) {
    ...
    nhPrivate.param("max_speed", max_speed_, 4.0);
    nhPrivate.param("min_speed", min_speed_, 0.22);
    nhPrivate.param("max_steering_angle", max_steering_angle_, 0.5);
    //wheel_base_
    nhPrivate.param("wheel_base", wheel_base_, 0.36);
    ...
    ros::Subscriber subPath = nh.subscribe<nav_msgs::Path>("/path_control", 5, pathHandler);
    ...
    while (status) {
        ...
        if (pathInit && has_odom_) {
            ...
            lookahead_point.position.x = vehicleX + path.poses[pathPointID].pose.position.x * cos(vehicleYaw)
                                         - path.poses[pathPointID].pose.position.y * sin(vehicleYaw);
            lookahead_point.position.y = vehicleY + path.poses[pathPointID].pose.position.x * sin(vehicleYaw)
                                         + path.poses[pathPointID].pose.position.y * cos(vehicleYaw);
            lookahead_point.position.z = vehicleZ + path.poses[pathPointID].pose.position.z;

            publishLookaheadMarker(lookahead_point);

            // Transform lookahead point to the vehicle's coordinate frame
            double dx = lookahead_point.position.x - vehicleX;
            double dy = lookahead_point.position.y - vehicleY;
            const double transformed_x = dx * cos(-vehicleYaw) - dy * sin(-vehicleYaw);
            const double transformed_y = dx * sin(-vehicleYaw) + dy * cos(-vehicleYaw);

            // Compute curvature
            const double curvature = 2.0 * transformed_y / (lookAheadDis * lookAheadDis);
            ...
            bool go_forward = transformed_x > 0; // If the lookahead point is in front of the vehicle
            double speed_ack;
            ...
            if (twoWayDrive) {
                ...
                if (!go_forward && navFwd && time - switchTime > switchTimeThre) {
                    navFwd = false;
                    switchTime = time;
                } else if (go_forward && !navFwd && time - switchTime > switchTimeThre) {
                    navFwd = true;
                    switchTime = time;
                }
            }
            ...
            if (navFwd) {
                // Scale velocity for forward motion
                double normalized_steering = std::abs(steering_angle) / max_steering_angle_; // 0.0 to 1.0
                speed_ack = max_speed_ * (1.0 - normalized_steering * (max_speed_ - min_speed_ / max_speed_)); // Scale between min_speed_ and max_speed_
                speed_ack = std::max(speed_ack, min_speed_); // Ensure speed is not below min_speed_
            } else {
                // Scale velocity for backward motion
                double normalized_steering = std::abs(steering_angle) / max_steering_angle_; // 0.0 to 1.0
                speed_ack = -max_speed_ * (1.0 - normalized_steering * (max_speed_ - min_speed_ / max_speed_)); // Scale between -min_speed_ and -max_speed_
                speed_ack = std::min(speed_ack, -min_speed_); // Ensure speed does not exceed -min_speed_
            }
            ...
            if (pathSize <= 1) {
                ...
                //speed_ack *= static_cast<float>(endDis / slowDwnDisThre);//
            }
            ...
            if (fabs(dirDiff) < dirDiffThre && dis > stopDisThre) {
                ...
                if (vehicleSpeed > 0) {
                    ...
                    //speed_ack -= static_cast<float>(maxAccel / 100.0);
                }
            ...
            //decrease speed to linearly to distance to goal
            if (dis < 3.0 * stopDisThre) {
                speed_ack = speed_ack * dis / (3.0 * stopDisThre);
                //if (vehicleSpeed > 0) vehicleSpeed -= static_cast<float>(maxAccel / 10.0);
                //else if (vehicleSpeed < 0) vehicleSpeed += static_cast<float>(maxAccel / 10.0);
                //speed_ack -= static_cast<float>(maxAccel / 100.0);
            }
            if (dis < 2.0 * stopDisThre) {
                speed_ack = speed_ack * dis / (5.0 * stopDisThre);
            }
            if (dis < stopDisThre) {
                speed_ack = speed_ack * dis / (10.0 * stopDisThre);
            }

            if (dis < stopDisThre * 0.5)
                speed_ack = 0;
            ...
            if (pubSkipCount < 0) {
                ...
                ackermann_msg.drive.speed = speed_ack; //linear_velocity;
