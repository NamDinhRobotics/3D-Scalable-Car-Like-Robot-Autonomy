...
std::mutex odom_mutex;   // Mutex for odometry data
std::mutex laser_mutex;  // Mutex for laser data
...
constexpr int terrainVoxelWidth = 21;
constexpr int terrainVoxelHalfWidth = (terrainVoxelWidth - 1) / 2;
constexpr int terrainVoxelNum = terrainVoxelWidth * terrainVoxelWidth;

// planar voxel parameters
constexpr float planarVoxelSize = 0.2;
constexpr int planarVoxelWidth = 51;
constexpr int planarVoxelHalfWidth = (planarVoxelWidth - 1) / 2;
constexpr int planarVoxelNum = planarVoxelWidth * planarVoxelWidth;
...
void odometryHandler(const nav_msgs::Odometry::ConstPtr &odom) {
    std::lock_guard<std::mutex> lock(odom_mutex); // Lock odometry data
    ...
    if (noDataInited == 1) {
        const float dis = sqrt((vehicleX - vehicleXRec) * (vehicleX - vehicleXRec) +
...
void laserCloudHandler(const sensor_msgs::PointCloud2ConstPtr &laserCloud2) {
    std::lock_guard<std::mutex> lock(laser_mutex); // Lock laser data
    ...
    const float terrainSizeH = terrainVoxelSize * (terrainVoxelHalfWidth + 1);
    ...
    for (int i = 0; i < laserCloudSize; i++) {
        ...
        const float dis = sqrt((pointX - vehicleX) * (pointX - vehicleX) +
...
int main(int argc, char **argv) {
    ...
    ros::Subscriber subLaserCloud = nh.subscribe<sensor_msgs::PointCloud2>(lidar_topic, 5, laserCloudHandler);
      ...
      newlaserCloud = false;
            ...
            while (vehicleX - terrainVoxelCenX < -terrainVoxelSize) {
                for (int indY = 0; indY < terrainVoxelWidth; indY++) {
                    ...
                    terrainVoxelCloud[indY]->clear(); // clear the pointer
                }
                terrainVoxelShiftX--;
                    ...
                    if (indX >= 0 && indX < planarVoxelWidth && indY >= 0 &&
                        ...
                        if (dis1 > minDyObsDis) {
                            ...
                            if (angle1 > minDyObsAngle) {
                                ...
                                if (angle4 > minDyObsVFOV && angle4 < maxDyObsVFOV || fabs(pointZ4) <
                                    absDyObsRelZThre) {
