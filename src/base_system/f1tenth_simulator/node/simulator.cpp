...
#include <cmath>
#include <random>
// Add this above main to include necessary headers
...
public:
    RacecarSimulator(): im_server("racecar_sim") {
        ...
        state = {
            .x = 0, .y = 0, .theta = 0, .velocity = 0, .steer_angle = 0.0, .angular_velocity = 0.0, .slip_angle = 0.0,
            .st_dyn = false
        };
        accel = 0.0;
        steer_angle_vel = 0.0;
        ...
        // Get the topic names
        std::string drive_topic, map_topic, scan_topic, pose_topic, gt_pose_topic, cloud_laser,
        ...
        // enable pub pcl
        n.getParam("enable_pcl", enable_pcl);
        n.getParam("cloud_laser", cloud_laser);
        ...
        // Make a publisher for point cloud messages
        pointcloud_pub = n.advertise<sensor_msgs::PointCloud2>(cloud_laser, 1);
    ...
    void update_pose(const ros::TimerEvent &) {
        ...
        if (map_exists) {
            ...
            if (enable_pcl) {
                ...
                std::normal_distribution<double> range_noise(0.0, range_noise_std);
                std::normal_distribution<double> angle_noise(0.0, angle_noise_std);

                // Fill the point cloud from the laser scan data, simulating 3D LiDAR
                for (size_t i = 0; i < scan_.size(); ++i) {
                    double angle = scan_msg.angle_min + i * scan_msg.angle_increment + angle_noise(rng);
                    if (scan_[i] > 0 && scan_[i] < scan_msg.range_max) {
                        double base_range = scan_[i] + range_noise(rng);

                        // For each horizontal angle, simulate vertical beams
                        for (int j = 0; j < num_vertical_beams; ++j) {
                            double vertical_angle = vertical_fov_min + j * vertical_angle_increment + angle_noise(rng);

                            // Calculate 3D point using spherical to Cartesian conversion
                            pcl::PointXYZI point;
                            point.x = scan_pose.x + base_range * cos(scan_pose.theta + angle) * cos(vertical_angle);
                            point.y = scan_pose.y + base_range * sin(scan_pose.theta + angle) * cos(vertical_angle);
                            point.z = base_range * sin(vertical_angle); // Vertical component

                            // Simulate intensity based on distance (optional)
                            point.intensity = scan_msg.intensities[i] * exp(-0.01 * base_range); // Exponential decay for intensity
    ...
    std::vector<int> ind_2_rc(const int ind) const {
        std::vector<int> rc;
        const int row = floor(ind / map_width);
        const int col = ind % map_width - 1;
        rc.push_back(row);
        rc.push_back(col);
        return rc;
    }

    int rc_2_ind(const int r, const int c) const {
        return r * map_width + c;
    }

    std::vector<int> coord_2_cell_rc(const double x, const double y) const {
        std::vector<int> rc;
    ...
    void set_accel(const double accel_) {
        accel = std::min(std::max(accel_, -max_accel), max_accel);
    }

    void set_steer_angle_vel(const double steer_angle_vel_) {
        steer_angle_vel = std::min(std::max(steer_angle_vel_, -max_steering_vel), max_steering_vel);
    }

    void add_obs(const int ind) {
        const std::vector<int> rc = ind_2_rc(ind);
    ...
    double compute_steer_vel(const double desired_angle) const {
    ...
    void compute_accel(const double desired_velocity) {
        // get difference between current and desired
        const double dif = (desired_velocity - state.velocity);

        if (state.velocity > 0) {
            if (dif > 0) {
                // accelerate
                const double kp = 2.0 * max_accel / max_speed;
                set_accel(kp * dif);
            } else {
                // brake
                accel = -max_decel;
    ...
    void obs_callback(const geometry_msgs::PointStamped &msg) {
        ...
        const std::vector<int> rc = coord_2_cell_rc(x, y);
    ...
    void map_callback(const nav_msgs::OccupancyGrid &msg) {
        ...
        const geometry_msgs::Quaternion q = msg.info.origin.orientation;
        const tf2::Quaternion quat(q.x, q.y, q.z, q.w);
    ...
    void pub_pose_transform(const ros::Time timestamp) {
    ...
    void pub_steer_ang_transform(const ros::Time timestamp) {
    ...
    void pub_laser_link_transform(const ros::Time timestamp) {
        // Publish a transformation between base link and laser
    ...
    void pub_odom(const ros::Time timestamp) const {
    ...
    void pub_imu(const ros::Time timestamp) const {
